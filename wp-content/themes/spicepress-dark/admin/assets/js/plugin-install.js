jQuery(document).ready(function($) {
    'use strict';
    var this_obj = spicepress_dark_companion_install;
    ...
    $(document).on('click', '.action-watch', function(event) {
        ...
        if(action_id){
            $.ajax({
                ...
                type: 'POST',
                data: {action: 'spicepress_dark_update_rec_acts', action_id:action_id},
                success:function(data) {
    ...
    $(document).on('click', '.spicethemes-customizer-notification-dismiss', function(event) {
        ...
        var sdata = {};
        if(slug){
            sdata.action = 'spicepress_dark_hide_customizer_notice';
            sdata.chilly_plugin_slug = slug;
        }else{
            sdata.action = 'spicepress_dark_hide_customizer_companion_notice';
        }
...
jQuery(function(){
	var $mainSpicePress_Dark=jQuery('#main-content');
	...
	$cat_links.on('click',function(e){
		...
		$mainSpicePress_Dark.animate({opacity:"0.5"});
		$mainSpicePress_Dark.load(value + "#getting_started",function(){
		$mainSpicePress_Dark.animate({opacity:"1"});	
		});
	});
});