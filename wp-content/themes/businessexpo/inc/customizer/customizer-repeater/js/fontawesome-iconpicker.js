(function ($) {
	'use strict';
	wp.businessexpo_customizerRepeater = {
		...
		init: function () {
			$( '.iconpicker-items>i' ).on(
				...
				function () {
					...
					businessexpo_customizer_repeater_refresh_social_icons( th );
					return false;
				}
			);
		},
	...
	$( document ).ready(
		function () {
			wp.businessexpo_customizerRepeater.init();

			$( '.iconpicker-search' ).on(
				'keyup',
				function () {
					wp.businessexpo_customizerRepeater.search( $( this ) );
				}
			);
			...
			$( '.icp-auto' ).on(
				'click',
				function () {
					wp.businessexpo_customizerRepeater.iconPickerToggle( $( this ) );
				}
			);
