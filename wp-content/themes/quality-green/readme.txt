=== Quality green ===

Contributors: priyanshumittal
Tags: full-width-template, translation-ready, theme-options, custom-menu, threaded-comments, two-columns, sticky-post, blog, footer-widgets, featured-images, right-sidebar, left-sidebar
Requires at least: 4.7
Tested up to: 5.8
Stable tag: 1.3.6
Requires PHP: 5.4
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

== Description ==
Quality Green is the child version of Quality theme.

== Installation ==

1. In your admin panel, go to Appearance > Themes and click the Add New button.
2. Click Upload and Choose File, then select the theme's .zip file. Click Install Now.
3. Click Activate to use your new theme right away.

== Keyboard navigation ==

Keyboard navigation is working properly in the forward direction by using the tab key. For the reverse direction, Shift+Tab key is working on all the parent menus and if the dropdown menus are open then it is working on it. The focus is moving back to the parent menu having submenus from the next menu in a reverse direction.

== License ==

Quality WordPress Theme, Copyright 2014-2021 priyanshumittal
Quality WordPress Theme is distributed under the terms of the GNU GPL

Quality Green WordPress Theme is a child theme of Quality Theme, Copyright (C) 2015-2021, priyanshumittal
Quality Green WordPress Theme is distributed under the terms of the GNU GPL

===== Changelog =========

== 1.3.6 =
1. Fixed WooCommerce pagination style issue.
== 1.3.5 =
1. Changed some customizer strings.
== 1.3.4 =
1. Fixed the theme check warnings.
== 1.3.3 =
1. Fixed the service section styling issue.
== 1.3.2 =
1. Fixed latest news section title and empty markup issues.
2. Added orginial file of mp.mansory.min.js file and removed unnecessary code.
3. Declared the license of mansory js. 
== 1.3.1 =
1. Added license of theme copyright and header radio button image.
2. Changed License & License URI in readme.
= 1.3 =
1. Implement the Accessibility including skip to content and keyboard navigation.
2. Changed the screenshot image.
3. Added the Header section variation, Service section variation & blog section variations.
4. Fixed escaping and sanitizing issues.
5. Fixed all the other issues.
= 1.2.8 =
1. Update String.
2. Add new layout header, service and blog.
= 1.2.7 =
1. Update String.
= 1.2.6 =
1. Update styling.
= 1.2.5 =
1. Strings update.
= 1.2.4 =
1. Strings update.
= 1.2.3 =
1. Updated Theme URI & Author URI
= 1.2.2 =
1. Remove duplicate fullwidth page.
= 1.2.1 =
1. Update pot file.
= 1.2 =
1. Change Screenshot Image.
= 1.1 =
1. Fixed Styling issue.
= 1.0.1 =
1 Added Theme URI
= 1.0 =
1. Added template Blog Full width
2. Added template Page Full Width
3. Added template fullwidth page

= 0.1 =
released

== External resources ==

Radio Image Control
Copyright: (c) Anthony Hortin
License: Under GNU General Public License
Source: https://github.com/maddisondesigns

Masonry JS:
Copyright: (C) 2007 Free Software Foundation
License: Under the GNU General Public License v3.0 
Source: https://github.com/jmlp-131092/mp-mansory.js

== Images ==

* Image used in the Screenshots
License CC0 Public Domain
https://pxhere.com/en/photo/714512

* Images used in Blog radio button
License CC0 Public Domain
https://pxhere.com/en/photo/1325885
https://pxhere.com/en/photo/1567131

* Image used in Header radio button
License CC0 Public Domain
https://pxhere.com/en/photo/1451901
