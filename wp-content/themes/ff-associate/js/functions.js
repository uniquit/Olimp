/* global ff_associateScreenReaderText */
/**
...
(function($) {
	...
	function initMainNavigation(container) {
		...
		var dropdownToggle = $('<button />', {
			...
			.append(ff_associateScreenReaderText.icon)
			.append($('<span />', {
				'class': 'screen-reader-text',
				text: ff_associateScreenReaderText.expand
			}));
		...
		container.find('.current-menu-ancestor > button')
			...
			.text(ff_associateScreenReaderText.collapse);
		...
		container.find('.dropdown-toggle').on( 'click', function(e) {
			...
			// jscs:enable
			screenReaderSpan.text(screenReaderSpan.text() === ff_associateScreenReaderText.expand ? ff_associateScreenReaderText.collapse : ff_associateScreenReaderText.expand);
		});
	}
	...
	(function() {
		...
		menuToggle.on('click.ff_associate', function() {
	...
	(function() {
		...
		function toggleFocusClassTouchScreen() {
			if (window.innerWidth >= 910) {
				$(document.body).on('touchstart.ff_associate', function(e) {
					if (!$(e.target).closest('.main-navigation li').length) {
						$('.main-navigation li').removeClass('focus');
					}
				});
				siteNavigation.find( '.menu-item-has-children > a, .page_item_has_children > a' ).on( 'touchstart.ff_associate', function( e ) {
				...
				siteNavigation.find('.menu-item-has-children > a').off('touchstart.ff_associate');
			}
		}

		if ('ontouchstart' in window) {
			$(window).on('resize.ff_associate', toggleFocusClassTouchScreen);
			toggleFocusClassTouchScreen();
		}

		siteNavigation.find('a').on('focus.ff_associate blur.ff_associate', function() {
	...
	$(document).ready(function() {
		$(window).on('load.ff_associate resize.ff_associate', function() {
			if (window.innerWidth < 910) {
