...
class FF_Associate_Customizer_Utilities {
	/**
	...
	public static function register_option( $option ) {
		...
		$defaults = apply_filters( 'ff_associate_customizer_defaults', array() );
		...
		if ( ! empty( $get_posts ) ) {
			foreach ( $get_posts as $get_post ) {
				...
				if ( empty( $title ) ) {
					...
					$title = sprintf( esc_html__( 'ID: %s No Title', 'ff-associate' ), $get_post->ID );
				}
	...
	static function section_visibility() {
		$options = array(
			'disabled'     => esc_html__( 'Disabled', 'ff-associate' ),
			'homepage'     => esc_html__( 'Homepage / Frontpage', 'ff-associate' ),
			'entire-site'  => esc_html__( 'Entire Site', 'ff-associate' ),
		);

		return apply_filters( 'ff_associate_section_visibility_options', $options );
	}
}
 ...
 * Initialize class
 */
$ff_associate_utilities = new FF_Associate_Customizer_Utilities();
